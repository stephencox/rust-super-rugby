...
) -> Result<()>
{
    ...
    for i in 0..runs {
        ...
        // Disable weight decay for linear model to be truly "simple"? 
        // Or keep it consistent? Let's keep it consistent.
        let trainer = MLPTrainer::<MyAutodiffBackend>::new(
            model,
            config.training.learning_rate,
            config.training.weight_decay,
            config.loss.win_weight,
            config.loss.score_weight,
            device,
        );

        print!("Run {}/{}: ", i + 1, runs);
        
        // Suppress logs by not initializing env_logger, or relying on it being Info by default
        // We can just capture the result.
...
fn main() -> Result<()>
{
    ...
    // Pass None for existing_mapping to allow automatic creation/mapping
    ...
    run_benchmark("Linear (Old)", &config, old_config, 10, &train_dataset, &val_dataset)?;
    ...
    run_benchmark("MLP (Improved)", &config, new_config, 10, &train_dataset, &val_dataset)?;
